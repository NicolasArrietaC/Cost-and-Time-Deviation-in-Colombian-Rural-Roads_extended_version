SELECT 
    -- CAST(identificacion_del_contratista AS STRING) || nom_razon_social_contratista AS ID_CONTRATISTA,
    identificacion_del_contratista AS ID_CONTRATISTA_NET,
    nom_razon_social_contratista AS NAME_CONTRATISTA_NET,
...
WHERE  
    (ID_CONTRATISTA_NET IN ({ID_CONTRATISTA}) OR NAME_CONTRATISTA_NET IN ({NAME_CONTRATISTA}))
    AND tipo_de_contrato = 'Obra'
    AND date_extract_y(fecha_de_firma_del_contrato) >= 2013
GROUP BY
    ID_CONTRATISTA_NET,
    NAME_CONTRATISTA_NET,
    MONTH_AT,
    YEAR_AT
LIMIT 20000
